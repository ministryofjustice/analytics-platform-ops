...
resource "aws_api_gateway_integration" "post_github" {
  ...
  uri = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${var.github_webhooks_handler_arn}/invocations"
  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Resource": "${var.github_webhooks_handler_arn}"
    }
  ]
}
EOF
