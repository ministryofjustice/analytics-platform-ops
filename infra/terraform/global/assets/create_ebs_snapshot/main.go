...
import (
	...
	"github.com/ministryofjustice/analytics-platform-ops/snapshot"
	"github.com/ministryofjustice/analytics-platform-ops/tag"
		...
		for _, instance := range res.Instances {
			...
			for _, blk := range instance.BlockDeviceMappings {
				...
				if isRootDevice != true {
					...
					snapShot, err := snapshot.Create(connection, volumeID)
					if err != nil {
						log.Printf("Error while taking snapshot of %s: %s", volumeID, err)
					}

					_, volumeName, err := tag.Fetch(connection, volumeID, "Name")
					if err != nil {
						log.Printf("Error while getting tags for volume %s: %s", volumeID, err)
					}

					_, instanceName, err := tag.Fetch(connection, instanceID, "Name")
					if err != nil {
					...
					tag.Resource(connection, snapShotID, Tags)
					if err != nil {
