...
class Role(object):
    ...
    def __init__(self, authz_api, authz_token, logger, role_data):
        if not role_data:
    ...
    def add_permission(self, permission_id):
        ...
        if resp.status_code != 200:
            self.log.error("Failed to add permission ({}) to role ({}): expected 200, got {}: {}".format(permission_id, self.id(), resp.status_code, resp.text))
            return None

        self.log.debug("Permission ({}) added to role ({})".format(permission_id, self.id()))
        self.data = resp.json()
        return self
